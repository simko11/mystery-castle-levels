scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.stairNorth, function on_overlap_tile(sprite: Sprite, location: tiles.Location) {
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function on_on_overlap(sprite3: Sprite, otherSprite: Sprite) {
    LifeLost()
})
controller.A.onEvent(ControllerButtonEvent.Pressed, function on_a_pressed() {
    ...
    if (Hero.tileKindAt(TileDirection.Right, assets.tile`
        myTile
    `)) {
    ...
    } else if (Hero.tileKindAt(TileDirection.Left, assets.tile`
        myTile
    `)) {
        Hero.setImage(assets.image`
            Attack Left
        ...
        tiles.setWallAt(tiles.getTileLocation(TileX - 1, TileY), false)
        info.changeScoreBy(1)
    } else if (Hero.tileKindAt(TileDirection.Top, assets.tile`
        myTile
    `)) {
        Hero.setImage(assets.image`
            Attack Up
        ...
        info.changeScoreBy(1)
    } else if (Hero.tileKindAt(TileDirection.Bottom, assets.tile`
        myTile
    `)) {
        Hero.setImage(assets.image`
            Attack Down
...
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.hazardLava0, function on_overlap_tile3(sprite4: Sprite, location3: tiles.Location) {
    LifeLost()
})
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.stairWest, function on_overlap_tile4(sprite5: Sprite, location4: tiles.Location) {
    
    Safe_at_ms = game.runtime()
    NextLevel()
})
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.chestClosed, function on_overlap_tile5(sprite22: Sprite, location22: tiles.Location) {
...
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.stairSouth, function on_overlap_tile6(sprite6: Sprite, location5: tiles.Location) {
...
game.onUpdateInterval(10000, function on_update_interval() {
    ...
    if (tiles.getTilesByType(sprites.dungeon.greenOuterNorth2).length > 0) {
        for (let index2 = 0; index2 < Ghost_Number; index2++) {
...
game.onUpdateInterval(10000, function on_update_interval2() {
    ...
    `).length > 0) {
        Ghost_2 = sprites.create(assets.image`
            skellyFront0
        `, SpriteKind.Enemy)
        animation.runImageAnimation(Ghost_2, assets.animation`
            enemy_2
        `, 100, true)
        tiles.placeOnRandomTile(Ghost_2, assets.tile`
            Spawn
        `)
